...
def main(opt):
    loader = BatchLoaderUnk(opt.tokens, opt.data_dir, opt.batch_size, opt.seq_length, opt.n_bpes)
...
if __name__=="__main__":
    ...
    parser.add_argument('--use_bpes', type=int, default=0, help='use words (1=yes)')
    ...
    parser.add_argument('--bpe_vec_size', type=int, default=200, help='dimensionality of word embeddings')
    parser.add_argument('--char_vec_size', type=int, default=200, help='dimensionality of character embeddings')
    ...
    parser.add_argument('--n_bpes', type=int, default=10000, help='max number of bpes in model')
    parser.add_argument('--n_chars', type=int, default=3000, help='max number of char in model')
    parser.add_argument('--top_chars', type=int, default=3000, help='top number of char in model')
    # bookkeeping
    ...
    parser.add_argument('--save_every', type=int, default=24, help='save every n epochs')
    ...
    assert params.use_bpes == 1 or params.use_bpes == 0, '-use_bpes has to be 0 or 1'
    assert params.use_chars == 1 or params.use_chars == 0, '-use_chars has to be 0 or 1'
    assert (params.use_chars + params.use_bpes) > 0, 'has to use at least one of words or chars'
