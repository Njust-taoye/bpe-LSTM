...
def LSTMCNN(opt):
    ...
    # opt.use_bpes = 1 if use word embeddings, otherwise not
    ...
    if opt.use_chars:
        ...
        chars = Input(batch_shape=(opt.batch_size, opt.seq_length, opt.n_chars), dtype='float32', name='chars')
        chars_dens = Dense(opt.char_vec_size, input_shape=(opt.seq_length, opt.n_chars))(chars)
        if opt.use_bpes:
            x = Concatenate()[chars, bpe_vecs]
            inputs = [chars, bpe]
        else:
            x = chars_dens 
            inputs = chars
