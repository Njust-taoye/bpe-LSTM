...
class sModel(Model):
    def fit_generator(self, generator, steps_per_epoch, epochs, train_sub, train_sub_steps, validation_data, validation_steps, opt, logfile):
        train_losses = []
        val_losses = []
        ...
        for epoch in range(epochs):
            ...
            train_loss = exp(self.evaluate_generator(train_sub, train_sub_steps))
            train_losses.append(train_loss)
            print('Epoch {}/{}. Train subset loss: {}'.format(epoch + 1, epochs, train_loss))
            print('Epoch {}/{}. Train subset loss: {}'.format(epoch + 1, epochs, train_loss), file=logfile)
            val_loss = exp(self.evaluate_generator(validation_data, validation_steps))
            val_losses.append(val_loss)
            print('Epoch {}/{}. Validataion loss: {}'.format(epoch + 1, epochs, val_loss))
            print('Epoch {}/{}. Validataion loss: {}'.format(epoch + 1, epochs, val_loss), file=logfile)
            ...
            if epoch == epochs-1 or epoch % opt.save_every == 0:
                savefile = '%s/lm_%s_epoch%d_%.2f.h5' % (opt.model_dir, opt.savefile, epoch + 1, val_loss)
...
def BPELSTM(opt):
    ...
    # opt.bpe_vocab_size = num words in the vocab
    ...
    if opt.predict_bpes:
        # 1-hot representation with embedding 
        bpe = Input(batch_shape=(opt.batch_size, opt.seq_length), dtype='int32', name='bpe')
        bpe_vecs = Embedding(opt.bpe_vocab_size, opt.bpe_vec_size, input_length=opt.seq_length)(bpe)
        
    if opt.predict_words:
        # letter representation without embedding
        bpes = Input(batch_shape=(opt.batch_size, opt.seq_length, opt.n_bpes), dtype='float32', name='bpes')
        bpes_dens = Dense(opt.bpe_vec_size, input_shape=(opt.seq_length, opt.n_bpes))(bpes)
        x = bpes_dens 
        inputs = bpes
    else:
        x = bpe_vecs # 1-hot with embedding 
        inputs = bpe 
    ...
    if opt.predict_bpes:
        output = TimeDistributed(Dense(opt.bpe_vocab_size, activation='softmax'))(x)
    if opt.predict_words:
